...
def generate_dataset_from_text(text):
    ...
    response = model.generate_content(["Generate a dataset in CSV format based on the following text without explanation, just data and I want 200 rows and 5 columns, avoid repeating of data both numeric as well as categorical also strictly don't give ```csv ``` or '''  ''' with dataset :", text])
